...
class ArchivesController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($this->request->isPost) {
            if ($model->load($this->request->post())) {
                ...
                try {
                    $poster = UploadedFile::getInstance($model, 'imageFile');
                    if ($poster) {
                        $upload = Gadget::yiiUploadFile($poster, Archives::UPLOAD_PATH, 'ARCHIVE_' . Jdf::jmktime());
                        if (!$upload['error']) {
                            $model->poster = $upload['path'];
                        }else {
                            $model->addError('imageFile', Message::UPLOAD_FAILED);
        ...
        if ($this->request->isPost && $model->load($this->request->post())) {
            try {
                ...
                if ($poster) {
                    $upload = Gadget::yiiUploadFile($poster, Archives::UPLOAD_PATH, 'ARCHIVE_' . Jdf::jmktime());
                    if (!$upload['error']) {
                        Gadget::deleteFile($model->poster, Archives::UPLOAD_PATH);
                        $model->poster = $upload['path'];
                    }else {
                        $model->addError('imageFile', Message::UPLOAD_FAILED);
    ...
    public function actionDelete($id)
    {
        ...
        if ($model->delete()) {
            if (ArchiveContent::deleteAll(['archive_id' => $id])) {
                $transaction->commit();
                Gadget::deleteFile($model->poster, Archives::UPLOAD_PATH);
            }else {
    ...
    {
        if (($model = Archives::findOne(['id' => $id])) !== null) {
            return $model;
        }
