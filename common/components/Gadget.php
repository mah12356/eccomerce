...
class Gadget
{
    ...
    public static function preview($data, $exit = true)
    {
        echo '<br>';
        echo '<pre>';
        echo 'INPUT => ';
...
//                Gadget::preview($folder , false );
//                Gadget::preview($fileName , false );
//                Gadget::preview($file , false );
//                Gadget::preview($upload);
            }
        } else {
    ...
    // آپلود فایل
    public static function phpUploadFile($file, $extensions, $name = null, $folder = 'tmp'): array
    {
        $extension = strtolower(pathinfo(Yii::getAlias('@upload') . '/' . basename($file['name']),PATHINFO_EXTENSION));
...
//        }
    ...
    //
    public static function fileExist($name, $folder = null): bool
    {
        if ($folder == null) {
    ...
    public static function validateMobileFormat($mobile): array
    {
        ...
        if ($numbers[0] == '#') {
            ...
            if (is_numeric($mobile) && count($numbers) == 11 && $numbers[1] == 9) {
                return ['error' => false, 'mobile' => $mobile, 'debug' => true];
            }
        }elseif (is_numeric($mobile) && count($numbers) == 11 && $numbers[0] == 0 && $numbers[1] == 9) {
            return ['error' => false, 'mobile' => $mobile, 'debug' => false];
        }
        return ['error' => true];
    }
    ...
    public static function JaliliDateToTimeStamp($date)
    {
